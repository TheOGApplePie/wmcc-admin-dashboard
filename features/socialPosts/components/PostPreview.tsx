...
function IgFeedPreview({ caption, mediaUrl }: Readonly<{ caption: string; mediaUrl: string | null }>) {
  return (
    <div
      className="flex flex-col rounded-xl overflow-hidden border"
      style={{ borderColor: "var(--sp-hairline)", maxWidth: 320, margin: "0 auto" }}
    >
      {/* Header */}
      <div className="flex items-center gap-2 px-3 py-2" style={{ backgroundColor: "var(--sp-surface)" }}>
        <div
          className="rounded-full flex items-center justify-center text-white text-[10px] font-bold"
          style={{ width: 28, height: 28, background: "linear-gradient(135deg, #C13584, #E0A53C)" }}
        >
          W
        </div>
        <div className="text-[12px] font-semibold" style={{ color: "var(--sp-ink)" }}>wmcc.ca</div>
        <div className="ml-auto text-[12px] font-semibold" style={{ color: "var(--sp-teal)" }}>Follow</div>
      </div>
      ...
      <div className="w-full" style={{ aspectRatio: "1/1", backgroundColor: "var(--sp-hairline)" }}>
        {mediaUrl ? (
          ...
          <div className="w-full h-full flex flex-col items-center justify-center gap-2">
            ...
            <span className="text-[11px]" style={{ color: "var(--sp-muted)" }}>No media</span>
          </div>
        )}
      </div>

      {/* Actions */}
      <div className="flex gap-3 px-3 pt-2" style={{ backgroundColor: "var(--sp-surface)" }}>
        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="var(--sp-ink)" strokeWidth="2">
      ...
      {/* Caption */}
      <div className="px-3 pt-1.5 pb-3" style={{ backgroundColor: "var(--sp-surface)" }}>
        {caption ? (
          <p className="text-[11px] line-clamp-3" style={{ color: "var(--sp-ink)" }}>
            <span className="font-semibold">wmcc.ca </span>
            {caption}
          </p>
        ) : (
          <p className="text-[11px] italic" style={{ color: "var(--sp-muted)" }}>Caption will appear here…</p>
        )}
      </div>
    </div>
  );
...
function IgStoryPreview({ caption, mediaUrl }: Readonly<{ caption: string; mediaUrl: string | null }>) {
  return (
    <div
    ...
    >
      ...
      <div className="relative flex items-center gap-1.5 px-2 pb-1">
        <div
          ...
          style={{ width: 20, height: 20, background: "linear-gradient(135deg, #C13584, #E0A53C)" }}
        >
          W
        </div>
...
function WhatsAppPreview({ caption, mediaUrl }: Readonly<{ caption: string; mediaUrl: string | null }>) {
    ...
    >
      ...
      <div className="flex items-center gap-2 px-4 py-3" style={{ backgroundColor: "#075E54" }}>
        <div
          ...
          style={{ width: 32, height: 32, backgroundColor: "#128C7E" }}
        >
          W
        </div>
        <div>
      ...
      <div className="p-4">
        <div className="rounded-xl rounded-tl-none overflow-hidden" style={{ backgroundColor: "var(--sp-surface)", maxWidth: "85%" }}>
          ...
          <div className="px-3 py-2">
            {caption ? (
              <p className="text-[12px]" style={{ color: "var(--sp-ink)" }}>{caption}</p>
            ) : (
              <p className="text-[12px] italic" style={{ color: "var(--sp-muted)" }}>Caption will appear here…</p>
            )}
            <div className="flex items-center justify-end gap-1 mt-1">
              <span className="text-[10px]" style={{ color: "var(--sp-muted)" }}>{timeStr}</span>
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="var(--sp-whatsapp)" strokeWidth="2.5">
...
export default function PostPreview({ channels, caption, mediaUrl }: Readonly<PostPreviewProps>) {
  ...
  if (channels.length === 0) {
    return (
      <div
        className="flex flex-col items-center justify-center rounded-2xl py-12 text-centre"
        style={{ backgroundColor: "var(--sp-canvas)", border: "1.5px dashed var(--sp-hairline)" }}
      >
        <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="var(--sp-muted)" strokeWidth="1.5" className="mb-2 opacity-50">
          <circle cx="12" cy="12" r="10" />
          <line x1="12" y1="8" x2="12" y2="12" />
          <line x1="12" y1="16" x2="12.01" y2="16" />
        </svg>
        <p className="text-[13px]" style={{ color: "var(--sp-muted)" }}>Select a channel to preview</p>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      {active === "ig_feed"  && <IgFeedPreview  caption={caption} mediaUrl={mediaUrl} />}
      {active === "ig_story" && <IgStoryPreview caption={caption} mediaUrl={mediaUrl} />}
      {active === "whatsapp" && <WhatsAppPreview caption={caption} mediaUrl={mediaUrl} />}
    </div>
  );
}
